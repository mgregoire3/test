...
class PhysicalQuantity(object):
    ...
    # from MG on 2020-Feb-27    
    def __gt__(self, other):
        # should add a check to make sure that the types are compatible ; actually probably in conversion_factor_to()
        # maybe no need for conversion_factor_to() in division
        if not isPhysicalQuantity(other):
            raise UnitError('Incompatible types')
        # if self.value / (other.value * other.unit.conversion_factor_to(self.unit)) < 1.0:
        if self.value > (other.value * other.unit.conversion_factor_to(self.unit)):
            # return other.value * other.unit.conversion_factor_to(self.unit)
            return True
        else: return False

    # from MG on 2020-Feb-27    
    def __lt__(self, other):
        # should add a check to make sure that the types are compatible
        if not isPhysicalQuantity(other):
            raise UnitError('Incompatible types')
        # if self.value / (other.value * other.unit.conversion_factor_to(self.unit)) > 1.0:
        if self.value < (other.value * other.unit.conversion_factor_to(self.unit)):
    ...
    def __max__(self, *args):
        ...
        for arg in args:
            # current_max = max(current_max, arg.value * arg.unit.conversion_factor_to(self.unit))
            if current_max < ( arg.value * arg.unit.conversion_factor_to(self.unit) ):
                current_max = arg.value
        return self.__class__(current_max, self.unit)
        # return self._sum(other, 1, 1)

    # from MG on 2020-Feb-27
    def __min__(self, *args):
        current_min = self.value
        for arg in args:
            # current_max = max(current_max, arg.value * arg.unit.conversion_factor_to(self.unit))
            if current_min > ( arg.value * arg.unit.conversion_factor_to(self.unit) ):
                current_min = arg.value
        return self.__class__(current_min, self.unit)
        # return self._sum(other, 1, 1)
